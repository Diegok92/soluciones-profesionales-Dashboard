...
function Rubros() {
  ...
  useEffect(() => {
    fetch("http://localhost:3001/rubros/api/professionals")
      ...
      .then((data) => {
        setInfoRequerida(data.data.infoProfession);
      });
  }, []);
    ...
    <div className="col-lg-6 mb-4">
      <div className="card shadow mb-4">
        <div className="card-header py-3">
          <h5 className="m-0 font-weight-bold text-gray-800">Rubros</h5>
        </div>

        <div>
          <div className="card-body">
            {infoRequerida.map((element, i) => {
              return (
                <div className="row" key={i}>
                  <div className="col-lg-6 mb-4">
                    <div className="card bg-dark text-white shadow">
                      <div className="card-body">
                        {element.Info.profession} {" - Total: " + element.total}
                      </div>
                    </div>
                  </div>
                </div>
